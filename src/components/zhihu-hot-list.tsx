...
export function ZhihuHotList() {
  ...
  return (
    <Panel title="求职热榜" subtitle="求职、简历、面试与申请经验。" aside={<Button variant="secondary" onClick={loadHot} busy={busy}>{items.length ? "刷新求职热榜" : "打开求职热榜"}</Button>}>
      {items.length === 0 && !error && <p className="text-sm text-slate-500">点一下，看看知乎上的求职经验。</p>}
