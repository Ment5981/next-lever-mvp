...
type MarketplaceTab = "jobs" | "candidates" | "activities";
...
function jobListings(state: WorkspaceState): JobListing[] {
  const real = state.jobs.map((job) => ({
    id: job.job_version_id,
    realJobVersionId: job.job_version_id,
    company: job.company_name,
    job,
    info: JOB_MARKET_INFO[job.job_version_id] ?? FALLBACK_INFO,
  }));
    ...
    <article className="mb-5 break-inside-avoid">
      <button type="button" onClick={onOpen} className="group block w-full text-left">
        ...
        <div className="px-1.5 pt-3">
          ...
          <div className="mt-3 flex flex-wrap gap-1.5 text-xs text-slate-400"><span className="rounded-full bg-slate-100 px-2 py-1">{listing.info.salary}</span><span className="rounded-full bg-slate-100 px-2 py-1">{listing.info.experience}</span></div>
        </div>
      </button>
    </article>
  );
...
function JobDetail({ listing, state, task, assessment, busy, notice, error, onClose, onStart }: { listing: JobListing; state: WorkspaceState; task: WorkspaceState["tasks"][number] | undefined; assessment: Assessment | undefined; busy: boolean; notice: string; error: string; onClose: () => void; onStart: () => void }) {
  ...
  return (
    <Dialog onClose={onClose} titleId="job-detail-title">
      ...
      <p className="mt-1 text-sm text-slate-500">{listing.company} · {info.subtitle}</p>
      ...
      {task && <div className="mt-4 rounded-2xl border border-emerald-200 bg-emerald-50 p-4"><div className="flex items-center justify-between gap-2"><p className="text-sm font-semibold text-emerald-950">已经聊过了</p><Badge tone="good">{stateText(task.state)}</Badge></div><p className="mt-2 text-sm leading-6 text-emerald-900">{task.messages.at(-1)?.text ?? "对话已完成"}</p><Link href="/a2a" className="mt-3 inline-block text-xs font-medium text-emerald-700 underline">查看完整 A2A 时间线 →</Link></div>}
      <div className="mt-6 flex flex-col gap-2 sm:flex-row sm:items-center"><div className="flex gap-2">{listing.realJobVersionId ? candidateReady ? <Button onClick={onStart} busy={busy} disabled={Boolean(task)}>{task ? "已完成 A2A 对话" : "和岗位 Agent 聊聊"}</Button> : <Link href="/candidate/materials" className="rounded-xl bg-indigo-600 px-4 py-2 text-center text-sm font-medium text-white hover:bg-indigo-500">先建立我的 Agent</Link> : <Button variant="secondary" disabled>演示岗位 · 即将开放</Button>}<button type="button" onClick={onClose} className="rounded-xl border border-slate-200 px-4 py-2 text-sm font-medium text-slate-700 hover:border-indigo-300">稍后再看</button></div>{notice && <Notice tone="good">{notice}</Notice>}{error && <Notice tone="warn">{error}</Notice>}</div>
    </Dialog>
  );
}
...
export function Marketplace({ initial }: { initial: WorkspaceState }) {
  ...
  const tab = tabOverride ?? (searchParams.get("tab") === "activities" ? "activities" : "jobs");
  ...
  const selectedCandidate = CANDIDATE_LISTINGS.find((candidate) => candidate.id === selectedCandidateId) ?? null;
  ...
  async function startA2A() {
    ...
    const dispatched = await callApi("/api/a2a/dispatch", {});
    if (dispatched.ok) { setNotice("A2A 对话已完成，反馈已经回到你的 Agent。"); await refresh(); } else setError(dispatched.blockers.join("、"));
    setBusy(false);
  }

  const tabItems: { id: MarketplaceTab; label: string; count: number }[] = [
    { id: "jobs", label: "岗位 Agent", count: jobs.length },
    { id: "candidates", label: "求职者 Agent", count: CANDIDATE_LISTINGS.length },
    { id: "activities", label: "成长活动", count: ACTIVITY_LISTINGS.length },
  ];

  return <div className="space-y-5"><div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><div className="flex items-center gap-2"><Badge tone="accent">求职广场</Badge><span className="text-xs text-slate-400">岗位、人才和成长活动</span></div><p className="mt-2 text-sm text-slate-500">先看清楚，再开始一段 Agent 对话或一次真实实践。</p></div><div className="flex flex-wrap gap-2"><Link href="/candidate/workbench" className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-700 hover:border-indigo-300">我的 Agent</Link><Link href="/employer" className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-700 hover:border-indigo-300">发布岗位</Link></div></div><div className="flex max-w-full gap-1 overflow-x-auto rounded-xl bg-slate-100 p-1">{tabItems.map((item) => <button key={item.id} type="button" onClick={() => setTabOverride(item.id)} className={`shrink-0 rounded-lg px-3 py-2 text-sm font-medium transition ${tab === item.id ? "bg-white text-slate-950 shadow-sm" : "text-slate-500 hover:text-slate-800"}`}>{item.label}<span className="ml-1 text-xs text-slate-400">{item.count}</span></button>)}</div>{tab === "jobs" && <div className="columns-1 gap-5 sm:columns-2 lg:columns-3">{jobs.map((listing, index) => <JobCard key={listing.id} listing={listing} index={index} assessment={listing.realJobVersionId ? state.assessments.find((item) => item.job_version_id === listing.realJobVersionId) : undefined} onOpen={() => { setSelectedJobId(listing.id); setError(""); setNotice(""); }} />)}</div>}{tab === "candidates" && <div className="columns-1 gap-5 sm:columns-2 lg:columns-3">{CANDIDATE_LISTINGS.map((candidate, index) => <CandidateCard key={candidate.id} candidate={candidate} index={index} onOpen={() => setSelectedCandidateId(candidate.id)} />)}</div>}{tab === "activities" && <div className="columns-1 gap-5 sm:columns-2 lg:columns-3">{ACTIVITY_LISTINGS.map((activity, index) => <ActivityCard key={activity.id} activity={activity} index={index} onOpen={() => setSelectedActivityId(activity.id)} />)}</div>}<p className="pt-2 text-center text-xs text-slate-400">点击卡片，查看详情</p>{selectedJob && <JobDetail listing={selectedJob} state={state} task={task} assessment={assessment} busy={busy} notice={notice} error={error} onClose={() => setSelectedJobId(null)} onStart={startA2A} />}{selectedCandidate && <CandidateDetail candidate={selectedCandidate} onClose={() => setSelectedCandidateId(null)} />}{selectedActivity && <ActivityDetail activity={selectedActivity} onClose={() => setSelectedActivityId(null)} />}</div>;
}
